...
import { auth, db } from "./firebase-config.js?v=128";
import {
...
import { loadUserProfile } from "./profile.js?v=128";
import { initNotifications, teardownNotifications } from "./notifications.js?v=128";
import { initFeed, initNearbyPlayers, teardownListeners } from "./feed.js?v=128";
import { goScreen, hideSplash } from "./ui.js?v=128";
      ...
      try {
        ...
        if (needsOnboard) {
          ...
          if (window._ssoOnboarding) {
            ...
            if (!alreadyOnOnboard) {
              // safeUI('goScreen','onboard') triggers buildOnboardScreen() in app.js
              if (typeof safeUI === 'function') safeUI('goScreen', 'onboard');
              else goScreen('onboard');
            }
            // onboard.js _wire() SSO handler will call _goTo(2)
          } else if (!alreadyOnOnboard) {
            // Fresh navigation to onboard — use safeUI so buildOnboardScreen() fires
            if (typeof safeUI === 'function') safeUI('goScreen', 'onboard');
            else goScreen('onboard');
          }
