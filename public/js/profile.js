...
import { VIBE_META, initials, avatarColor, showToast } from "./ui.js?v=125";
...
export async function saveProfileData({ bio, gender, city, homeCourse, handicap, ageRange, lat, lon }) {
  const user = window._currentUser;
  if (!user) throw new Error("Not signed in.");
...
export function updateProfileUI() {
  ...
  set("profile-gender",     myProfile.gender     || "—");
...
export async function downgradeSubscription() {
  ...
  try {
    ...
    const { updateProfileUI } = await import('./profile.js?v=125');
    updateProfileUI();
  } catch(e) {
